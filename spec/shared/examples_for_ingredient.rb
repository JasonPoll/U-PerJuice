RSpec.shared_examples "an Ingredient derivation" do
  subject { described_class.new(name:             "Blueberry(#{described_class.name}-#{SecureRandom.uuid})", 
  ...
  context 'validations:' do
    ...
    it 'requires .name to be unique (case insensitive)' do 
      expect(subject).to be_valid
      subject.save!
      ing2 = described_class.new(subject.attributes.merge({name: subject.name.upcase}))
    ...
    context 'after_initialize' do
      ...
      context '#ensure_default_values_are_set' do
        ...
        it 'really is called after_initialize' do 
          ing = described_class.allocate # like .new, but without calling #initialize
